...
module.exports = async ({ tarballDir, pkgRoot }, { cwd, env, stdout, stderr, nextRelease, logger }) => {
  const basePath = pkgRoot ? path.resolve(cwd, pkgRoot) : cwd;

  logger.log("Write version %s to package.json in %s", nextRelease.version, basePath);

  const versionResult = execa("yarn", ["version", nextRelease.type], { cwd: basePath, env });
  ...
  if (tarballDir) {
    ...
    await move(path.resolve(cwd, "package.tgz"), path.resolve(cwd, tarballDir.trim(), "package.tgz"));
  }
};
